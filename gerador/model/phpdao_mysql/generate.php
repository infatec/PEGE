...
function init(){
	...
	copy('../../../../models/class/sql/ConnectionProperty.class.php', 'generated/class/sql/ConnectionProperty.class.php');
...
function createIncludeFile($ret){
	...
	for($i=0;$i<count($ret);$i++){
		...
		$str .= "\trequire_once('class/mysql/".$clazzName."DAO.class.php');\n";
		$str .= "\trequire_once('class/mysql/ext/".$clazzName."ExtDAO.class.php');\n";
	}
	$template = new Template('templates/include_dao.tpl');
	$template->set('include', $str);
	$template->write('generated/include_dao.php');
    $template->write('../../../models/include_dao.php');
}
...
function createDAOFactory($ret){
    ...
    $template->write('../../../models/class/dao/DAOFactory.class.php');
}
...
function getnerateDAOExtObjects($ret){
	for($i=0;$i<count($ret);$i++){
		...
		for($j=0;$j<count($tab);$j++){
			if($tab[$j][3]=='PRI'){
				...
				if(isColumnTypeNumber($tab[$j][1])){
					$parameterSetter .= "\t\t\$sqlQuery->setNumber($".getVarName($tableName)."->".getVarName($tab[$j][0]).");\n";
                    $parameterSetter_up .= "\t\t\if(!empty($".getVarName($tableName)."->".getVarName($tab[$j][0]).")) $sqlQuery->setNumber($".getVarName($tableName)."->".getVarName($tab[$j][0]).");\n";
                    $parameter_setter_str .= "\t\t\if(!empty($".getVarName($tableName)."->".getVarName($tab[$j][0]).")) $campos .=' ".$tab[$j][0]." = ?,';\n";
				}else{
					$parameterSetter .= "\t\t\$sqlQuery->set($".getVarName($tableName)."->".getVarName($tab[$j][0]).");\n";
                    $parameterSetter_up .= "\t\t\if(!empty($".getVarName($tableName)."->".getVarName($tab[$j][0]).")) $sqlQuery->setNumber($".getVarName($tableName)."->".getVarName($tab[$j][0]).");\n";
                    $parameter_setter_str .= "\t\t\if(!empty($".getVarName($tableName)."->".getVarName($tab[$j][0]).")) $campos .=' ".$tab[$j][0]." = ?,';\n";
				}
				$parameterSetter2 = '';
		...
		return \$this->executeUpdate(\$sqlQuery);
			...
			$readRow .= "\t\t\$".getVarName($tableName)."->".getVarName($tab[$j][0])." = \$row['".$tab[$j][0]."'];\n";
		}
		if($pk==''){
			continue;
		}
		...
		$template->set('pk', $pk);
		$template->set('pk_php', getVarName($pk));		
        ...
        $template->set('parameter_setter_up',$parameterSetter_up);
        $template->set('parameter_setter_str',$parameter_setter_str);
		$template->set('read_row',$readRow);
		$template->set('date', date("Y-m-d H:i"));
        ...
        $file = '../../../models/class/mysql/ext/'.$clazzName.'DAO.class.php';
		if(!file_exists($file)){
			$template->write('../../../models/class/mysql/ext/'.$clazzName.'DAO.class.php');
		}
	}
}
...
function getnerateDAOObjects($ret){
	for($i=0;$i<count($ret);$i++){
		...
		for($j=0;$j<count($tab);$j++){
			if($tab[$j][3]=='PRI'){
				...
				if(isColumnTypeNumber($tab[$j][1])){
					...
					$parameterSetter .= "\t\t\$sqlQuery->set($".getVarName($tableName)."->".getVarName($tab[$j][0]).");\n";
                    $parameterSetter_up .= "\t\t if(!empty($".getVarName($tableName)."->".getVarName($tab[$j][0]).")) \t\t\$sqlQuery->set($".getVarName($tableName)."->".getVarName($tab[$j][0]).");\n";
                    $parameter_setter_str .= "\t\t if(!empty($".getVarName($tableName)."->".getVarName($tab[$j][0]).")) \$campos .=' ".$tab[$j][0]." = ?,';\n";
				}
				$parameterSetter2 = '';
				if(isColumnTypeNumber($tab[$j][1])){
		...
		for($z=0;$z<count($pks);$z++){
			...
			$s3 .= "\n";
			$s4 .= "\n\t\t";
			$s4 .= '$sqlQuery->setNumber($'.getVarName($tableName).'->'.getVarName($pks[$z]).');';
			$s4 .= "\n";
		}
		if($s[0]==',')$s = substr($s,1);
        ...
        $template->write('../../../models/class/mysql/'.$clazzName.'DAO.class.php');
	}
}
...
function getnerateIDAOObjects($ret){
	for($i=0;$i<count($ret);$i++){
		...
		for($z=0;$z<count($pks);$z++){
			...
			$insertFields2.=', '.getVarName($pks[$z]);
			$s .= '$'.getVarName($pks[$z]);
			$s2 .= getVarName($pks[$z]).' = ? ';
			$s3 .= '$sqlQuery->setNumber('.getVarName($pks[$z]).');';			
			$s3 .= "\n";
        ...
        $template->write('../../../models/class/dao/'.$clazzName.'DAO.class.php');
	}
}
