<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getModalidadeEnsinoMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $modalidade_ensino_mecs=array();
        foreach($id as $dados)
        {
           $modalidade_ensino_mec = DAOFactory::getModalidadeEnsinoMecDAO()->load($dados);
           if(empty($modalidade_ensino_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $modalidade_ensino_mecs[]=$modalidade_ensino_mec;
        }            
    }
    else
	{
