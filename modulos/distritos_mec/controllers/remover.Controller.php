<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getDistritosMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $distritos_mecs=array();
        foreach($id as $dados)
        {
           $distritos_mec = DAOFactory::getDistritosMecDAO()->load($dados);
           if(empty($distritos_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $distritos_mecs[]=$distritos_mec;
        }            
    }
    else
	{
