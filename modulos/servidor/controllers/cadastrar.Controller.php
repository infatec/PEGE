<?php
    ...
    if($_POST["acao"])
    {
        $not_validacao=array('id','email','numero','local','telefone','cep','rg','orgao_emissor','estado_emissor','pis_pasep','titulo_eleitor_numero','zona','secao',
        ...
        'carteira_trab','carteira_serie','data_nascimento');//AQUI VOU COLOCAR OS CAMPOS QUE NÃO PRECISAM SER VALIDADOS
        #### faço a validação #####
        ...
        $servidor->professor =$_POST['professor'];
        ...
        $servidor->dataNascimento = geradatamy($_POST['data_nascimento']);
        ...
        if(empty($erro))
        {
            $servidor->senha = geraSenha($_POST["senha"]);
            ...
            $servidor->created=date('Y-m-d H:i:s');
            $servidor->status=1;
            $servidor_id=DAOFactory::getServidorDAO()->insert($servidor);
    ...
    if(!is_array($escolas_selecionadas)) $escolas_selecionadas=array();
?>