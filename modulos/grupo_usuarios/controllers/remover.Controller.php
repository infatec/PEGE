<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getGrupoUsuariosDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $grupo_usuarioss=array();
        foreach($id as $dados)
        {
           $grupo_usuarios = DAOFactory::getGrupoUsuariosDAO()->load($dados);
           ...
           $grupo_usuarioss[]=$grupo_usuarios;
        }            
    }
    else
	{
