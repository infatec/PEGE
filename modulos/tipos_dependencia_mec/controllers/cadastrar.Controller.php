<?php
    ...
    if($_POST["acao"])
    {
		...
		$tipos_dependencia_mec->individualEscola = $_POST['individual_escola'];

        if(empty($erro))
        {
            $tipos_dependencia_mec->created=date('Y-m-d H:i:s');
            $tipos_dependencia_mec->status=1;
            DAOFactory::getTiposDependenciaMecDAO()->insert($tipos_dependencia_mec);
