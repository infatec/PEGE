<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getTiposDependenciaMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $tipos_dependencia_mecs=array();
        foreach($id as $dados)
        {
           $tipos_dependencia_mec = DAOFactory::getTiposDependenciaMecDAO()->load($dados);
           if(empty($tipos_dependencia_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $tipos_dependencia_mecs[]=$tipos_dependencia_mec;
        }            
    }
    else
	{
