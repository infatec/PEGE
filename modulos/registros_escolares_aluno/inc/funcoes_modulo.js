...
function getAluno(){
    ...
    $( "#suggest_aluno" ).autocomplete({
        source: "ajax/suggest_aluno.php?escola_id="+escola_id+"&ano_letivo_id="+ano_letivo_id,
        minLength: 3,
        select: function( event, ui ) {
            $(this).attr('cod_aluno',ui.item.id);
            RegistrosAluno(ui.item.id);
        }
    });

}

function RegistrosAluno(cod_aluno){
