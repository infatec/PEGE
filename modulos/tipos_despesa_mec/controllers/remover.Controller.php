<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getTiposDespesaMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $tipos_despesa_mecs=array();
        foreach($id as $dados)
        {
           $tipos_despesa_mec = DAOFactory::getTiposDespesaMecDAO()->load($dados);
           if(empty($tipos_despesa_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $tipos_despesa_mecs[]=$tipos_despesa_mec;
        }            
    }
    else
	{
