<?php
    ...
    if($_POST["acao"])
    {
		...
		$tipos_bem_mec->individualEscola = $_POST['individual_escola'];

        if(empty($erro))
        {
            $tipos_bem_mec->created=date('Y-m-d H:i:s');
            $tipos_bem_mec->status=1;
            DAOFactory::getTiposBemMecDAO()->insert($tipos_bem_mec);
            $tipos_bem_mec=NULL;
