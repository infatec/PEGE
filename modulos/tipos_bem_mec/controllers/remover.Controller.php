<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getTiposBemMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $tipos_bem_mecs=array();
        foreach($id as $dados)
        {
           $tipos_bem_mec = DAOFactory::getTiposBemMecDAO()->load($dados);
           if(empty($tipos_bem_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $tipos_bem_mecs[]=$tipos_bem_mec;
        }            
    }
    else
	{
