<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getNivelEnsinoMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $nivel_ensino_mecs=array();
        foreach($id as $dados)
        {
           $nivel_ensino_mec = DAOFactory::getNivelEnsinoMecDAO()->load($dados);
           if(empty($nivel_ensino_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $nivel_ensino_mecs[]=$nivel_ensino_mec;
        }            
    }
    else
	{
