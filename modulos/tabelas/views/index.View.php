	...
	<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
        <tr> 
            ...
            <td valign="top">
				...
				<option value='tabela_id' <? if(trim($campo)==trim($campo_sel)) echo'selected';?>>Tabela</option>
				<option value='menu_id' <? if(trim($campo)==trim($campo_sel)) echo'selected';?>>Menu</option>
				<option value='nome' <? if(trim($campo)==trim($campo_sel)) echo'selected';?>>Nome</option>
				<option value='pasta' <? if(trim($campo)==trim($campo_sel)) echo'selected';?>>Pasta</option>
		...
		<th>Pasta</th>
    						...
    						foreach($tabelass as $obj):
							    ...
							    if ($_SESSION["permissao_temp"]>1) $opcoes.= '<a href="atualizar.php?id='.$id.'"><img src="../../webroot/img_sistema/icon_editar.png" border="0" alt="Editar" title="Editar" width="23" /></a>&nbsp;&nbsp;';
						          ...
						          $tabela = DAOFactory::getTabelasDAO()->toString('tabelas','nome',$obj->tabelaId);	
                                  $menu = DAOFactory::getTabelasDAO()->toString('menus','nome',$obj->menuId);
                            ?>    
    							<tr bgcolor="">
						...
						<td class='text_padrao'><?=$tabela?></td>
						<td class='text_padrao'><?=$menu?></td>
												<td class='text_padrao'><?=$obj->nome?></td>
												<td class='text_padrao'><?=$obj->pasta?></td>
