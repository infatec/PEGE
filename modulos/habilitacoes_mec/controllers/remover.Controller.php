<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getHabilitacoesMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $habilitacoes_mecs=array();
        foreach($id as $dados)
        {
           $habilitacoes_mec = DAOFactory::getHabilitacoesMecDAO()->load($dados);
           if(empty($habilitacoes_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $habilitacoes_mecs[]=$habilitacoes_mec;
        }            
    }
    else
	{
