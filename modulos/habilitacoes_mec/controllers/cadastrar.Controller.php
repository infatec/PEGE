<?php
    ...
    if($_POST["acao"])
    {
		...
		$habilitacoes_mec->individualEscola = $_POST['individual_escola'];

        if(empty($erro))
        {
            $habilitacoes_mec->created=date('Y-m-d H:i:s');
            $habilitacoes_mec->status=1;
            DAOFactory::getHabilitacoesMecDAO()->insert($habilitacoes_mec);
