<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getTiposMaterialMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $tipos_material_mecs=array();
        foreach($id as $dados)
        {
           $tipos_material_mec = DAOFactory::getTiposMaterialMecDAO()->load($dados);
           if(empty($tipos_material_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $tipos_material_mecs[]=$tipos_material_mec;
        }            
    }
    else
	{
