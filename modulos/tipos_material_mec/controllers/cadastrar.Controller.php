<?php
    ...
    if($_POST["acao"])
    {
		...
		$tipos_material_mec->individualEscola = $_POST['individual_escola'];

        if(empty($erro))
        {
            $tipos_material_mec->created=date('Y-m-d H:i:s');
            $tipos_material_mec->status=1;
            DAOFactory::getTiposMaterialMecDAO()->insert($tipos_material_mec);
