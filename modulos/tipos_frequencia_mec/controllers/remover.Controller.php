<?php
    ...
    if($_POST["acao"])
    {
        ...
        foreach($id as $value)
        {
            $result=DAOFactory::getTiposFrequenciaMecDAO()->delete($value);
            if(!$result) 
            {
                $trans->rollback();
                $erro=1;
    ...
    {
        $tipos_frequencia_mecs=array();
        foreach($id as $dados)
        {
           $tipos_frequencia_mec = DAOFactory::getTiposFrequenciaMecDAO()->load($dados);
           if(empty($tipos_frequencia_mec)) 
           {
             $_SESSION["msg_index"] = 'Registro não existe.';
             redireciona("index.php");
             exit;   
           }
           $tipos_frequencia_mecs[]=$tipos_frequencia_mec;
        }            
    }
    else
	{
