...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 title: false,
			 pagestat: 'Resultados {from} para {to} de {total} itens',
			 procmsg: 'Aguarde enquanto est&aacute; processando ...',
			 query: '',
			 qtype: '',
			 nomsg: 'No items',
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'> Busca R&aacute;pida <input type='text' size='30' name='q' class='qsbox' /><input type='button' id='buscar' value='buscar' /> <select name='qtype'>"+sopt+"</select> <input type='button' id='Clear' value='Limpar' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[id=buscar]',g.sDiv).click(function(){g.doSearch(); });
